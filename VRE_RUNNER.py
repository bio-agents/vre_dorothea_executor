...
from agent.VRE_Agent import dorotheaAgent
...
class Wrapper:
    ...
    def __init__(self, configuration=None):
        ...
        :param configuration: A dictionary containing parameters that define how the operation should be carried out,
            which are specific to DoRothEA agent.
    ...
    def run(self, input_files, input_metadata, output_files, output_metadata):
        """
        Main run function for running DoRothEA agent.
...
def main_wrapper(config_path, in_metadata_path, out_metadata_path):
    ...
    try:
        ...
        logger.progress("DoRothEA agent successfully executed; see {}".format(out_metadata_path))
        return result

    except Exception as error:
        errstr = "DoRothEA agent wasn't successfully executed. ERROR: {}.".format(error)
        logger.error(errstr)
        raise Exception(errstr)


if __name__ == "__main__":
    PARSER = argparse.ArgumentParser(description="VRE DoRothEA Agent")
